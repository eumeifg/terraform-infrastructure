...
inputs = {
  ...
  iam_assumable_roles_with_oidc_and_policies = {
    ...
    social-reputation = {
      role_name = "social-reputation-${dependency.eks.outputs.eks.cluster_id}"

      oidc_fully_qualified_subjects = ["system:serviceaccount:social-reputation:social-reputation"]

      iam_policy = {
        name   = "social-reputation-${dependency.eks.outputs.eks.cluster_id}"
