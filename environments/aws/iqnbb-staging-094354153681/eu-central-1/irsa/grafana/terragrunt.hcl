...
inputs = {
  ...
  iam_assumable_roles_with_oidc_and_policies = {
    ...
    grafana = {
      role_name = "grafana-${dependency.eks.outputs.eks.cluster_id}"

      oidc_fully_qualified_subjects = ["system:serviceaccount:monitoring:kube-prometheus-stack-grafana"]

      iam_policy = {
        name   = "grafana-${dependency.eks.outputs.eks.cluster_id}"
