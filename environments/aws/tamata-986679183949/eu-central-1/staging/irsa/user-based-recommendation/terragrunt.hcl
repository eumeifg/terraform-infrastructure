...
inputs = {
  ...
  iam_assumable_roles_with_oidc_and_policies = {
    ...
    karpenter = {
      role_name = "user-based-recommendation-${dependency.eks.outputs.eks.cluster_id}"

      oidc_fully_qualified_subjects = ["system:serviceaccount:tamata:user-based-recommendation"]

      iam_policy = {
        name   = "user-based-recommendation-${dependency.eks.outputs.eks.cluster_id}"
