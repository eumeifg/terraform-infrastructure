...
inputs = {
  ...
  iam_assumable_roles_with_oidc_and_policies = {
    ...
    karpenter = {
      role_name = "fashion-recommender-${dependency.eks.outputs.eks.cluster_id}"

      oidc_fully_qualified_subjects = ["system:serviceaccount:tamata:fashion-recommender"]

      iam_policy = {
        name   = "fashion-recommender-${dependency.eks.outputs.eks.cluster_id}"
