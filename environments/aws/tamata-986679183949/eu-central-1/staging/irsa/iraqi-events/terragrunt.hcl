...
inputs = {
  ...
  iam_assumable_roles_with_oidc_and_policies = {
    ...
    iraqi-events = {
      role_name = "iraqi-events-${dependency.eks.outputs.eks.cluster_id}"

      oidc_fully_qualified_subjects = ["system:serviceaccount:tamata:iraqi-events"]

      iam_policy = {
        name   = "iraqi-events-${dependency.eks.outputs.eks.cluster_id}"
