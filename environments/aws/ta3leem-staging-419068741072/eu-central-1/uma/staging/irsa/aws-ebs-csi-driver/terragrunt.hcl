...
inputs = {
  ...
  iam_assumable_roles_with_oidc_and_policies = {
    ...
    aws-ebs-csi-driver = {
      role_name = "aws-ebs-csi-driver-${dependency.eks.outputs.eks.cluster_id}"

      oidc_fully_qualified_subjects = ["system:serviceaccount:kube-system:aws-ebs-csi-driver"]

      iam_policy = {
        name   = "aws-ebs-csi-driver-${dependency.eks.outputs.eks.cluster_id}"
