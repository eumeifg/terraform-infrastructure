...
inputs = {
  ...
  iam_assumable_roles_with_oidc_and_policies = {
    ...
    uma-hrms = {
      role_name = "uma-hrms-${dependency.eks.outputs.eks.cluster_id}"

      oidc_fully_qualified_subjects = [
	"system:serviceaccount:uma:uma-hrms",
	"system:serviceaccount:uma-demo:uma-hrms"
      ]

      iam_policy = {
        name   = "uma-hrms-${dependency.eks.outputs.eks.cluster_id}"
