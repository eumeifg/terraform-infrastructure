...
inputs = {
  ...
  iam_assumable_roles_with_oidc_and_policies = {
    ...
    cloudwatch-exporter = {
      role_name = "cloudwatch-exporter-${dependency.eks.outputs.eks.cluster_id}"

      oidc_fully_qualified_subjects = ["system:serviceaccount:monitoring:cloudwatch-exporter"]

      iam_policy = {
        name   = "cloudwatch-exporter-${dependency.eks.outputs.eks.cluster_id}"
