...
dependency "eks" {
  config_path = "../../../../infra-614592275287/eu-central-1/eks/"
}
...
inputs = {
  ...
  iam_assumable_roles_with_oidc_and_policies = {
    atlantis-irsa = {
      role_name = "atlantis-irsa-${dependency.eks.outputs.eks.cluster_id}"

      oidc_fully_qualified_subjects = ["system:serviceaccount:atlantis:atlantis"]

      iam_policy = {
        name   = "atlantis-irsa-${dependency.eks.outputs.eks.cluster_id}"
        policy = file("policies/atlantis-irsa.json")
      }
    }
  }
