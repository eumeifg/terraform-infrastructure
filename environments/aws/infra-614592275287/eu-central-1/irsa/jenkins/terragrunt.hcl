...
inputs = {
  ...
  iam_assumable_roles_with_oidc_and_policies = {
    ...
    grafana = {
      role_name = "jenkins-${dependency.eks.outputs.eks.cluster_id}"

      oidc_fully_qualified_subjects = ["system:serviceaccount:infra:jenkins"]

      iam_policy = {
        name   = "jenkins-${dependency.eks.outputs.eks.cluster_id}"
