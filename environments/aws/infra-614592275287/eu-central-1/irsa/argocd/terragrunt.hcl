...
inputs = {
  ...
  iam_assumable_roles_with_oidc_and_policies = {
    ...
    argocd-serviceaccount = {
      role_name = "argocd-serviceaccount-${dependency.eks.outputs.eks.cluster_id}"

      oidc_fully_qualified_subjects = ["system:serviceaccount:argocd:argocd-serviceaccount"]

      iam_policy = {
        name   = "argocd-serviceaccount-${dependency.eks.outputs.eks.cluster_id}"
