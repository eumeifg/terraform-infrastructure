...
inputs = {
  ...
  iam_assumable_roles_with_oidc_and_policies = {
    ...
    mongodb-backup = {
      role_name = "mongodb-backup-${dependency.eks.outputs.eks.cluster_id}"

      oidc_fully_qualified_subjects = ["system:serviceaccount:vpn:mongodb-backup"]

      iam_policy = {
        name   = "mongodb-backup-${dependency.eks.outputs.eks.cluster_id}"
