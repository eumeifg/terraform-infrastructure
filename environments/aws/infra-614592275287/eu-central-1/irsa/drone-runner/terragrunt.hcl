...
inputs = {
  ...
  iam_assumable_roles_with_oidc_and_policies = {
    ...
    drone-runner = {
      role_name = "drone-runner-${dependency.eks.outputs.eks.cluster_id}"

      oidc_fully_qualified_subjects = ["system:serviceaccount:drone:drone-runner-kube"]

      iam_policy = {
        name   = "drone-runner-${dependency.eks.outputs.eks.cluster_id}"
