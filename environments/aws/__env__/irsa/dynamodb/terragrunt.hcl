...
inputs = {
  ...
  iam_assumable_roles_with_oidc_and_policies = {
    ...
    dynamodb = {
      role_name = "dynamobdb-${dependency.eks.outputs.eks.cluster_id}"

      oidc_fully_qualified_subjects = ["system:serviceaccount:kube-system:dynamodb"]

      iam_policy = {
        name   = "dynamodb-${dependency.eks.outputs.eks.cluster_id}"
