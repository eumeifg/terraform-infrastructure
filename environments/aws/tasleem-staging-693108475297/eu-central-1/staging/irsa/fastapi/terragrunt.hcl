...
inputs = {
  ...
  iam_assumable_roles_with_oidc_and_policies = {
    ...
    fastapi = {
      role_name = "fastapi-${dependency.eks.outputs.eks.cluster_id}"

      oidc_fully_qualified_subjects = ["system:serviceaccount:tasleem:fastapi", "system:serviceaccount:tasleem-dev:fastapi"]

      iam_policy = {
        name   = "fastapi-${dependency.eks.outputs.eks.cluster_id}"
