...
inputs = {
  ...
  iam_assumable_roles_with_oidc_and_policies = {
    ...
    mysql-backup = {
      role_name = "mysql-backup-${dependency.eks.outputs.eks.cluster_id}"

      oidc_fully_qualified_subjects = ["system:serviceaccount:tasleem:mysql-backup", "system:serviceaccount:tasleem-dev:mysql-backup"]

      iam_policy = {
        name   = "mysql-backup-${dependency.eks.outputs.eks.cluster_id}"
