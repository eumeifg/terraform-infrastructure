...
generate "main_providers" {
  ...
  contents  = <<-EOT
    ...
    variable "role_arn" {
      ...
      type        = string
      default     = "arn:aws:iam::310830963532:role/DevOps"
    }
    ...
    variable "common_tags" {
      description = "Map of common tags shared across all infrastructure resources (eg, Environment name)"
      type        = map(string)
      default     = {}
    }

    provider "vault" {
      address = "https://vault.creativeadvtech.ml/"
    }
  EOT
}
