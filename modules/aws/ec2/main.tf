...
resource "aws_eip" "this_spot" {
  count    = local.create && var.create_spot_instance && var.create_spot_instance_public ? 1 : 0
  instance = aws_spot_instance_request.this[count.index].id
  vpc      = true
  tags     = {
    "Name" = "${var.name}-IP"
  }
